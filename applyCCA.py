...
class ccaModel:
	def __init__(self, collection, feature, path=ROOT_PATH, modelfile='cca.model'):
		...
		self.tagIdxMapping = utilCCA.getTagIdxMapping('freqtags.txt')
	...
	def T2I_get_top_n_images(self, tags, top_n = 5):
		...
		for (k,v) in self.idx_mapping.items():
			#if k == img_id:continue
			curr_visual = self.visual_feature[self.idx_mapping[k]]
			similarity = self.compute_similarity(home_textual, 2, curr_visual, 1)
			leadboard.append((k,similarity))
			count += 1
			if count % 2000 == 0:
	...
	def I2I_check_correctness(self, requested, topList):
		...
		print "%s truth concepts:"%requested, truth_concepts[1]
		for (img_id, _) in topList:
			concepts = mymodel.get_ground_truth(img_id)
	...
	def get_ground_truth(self, requested):
		concepts = []
		curr_semantic = self.semantic_feature[self.idx_mapping[requested]]
	...
	def kernel_mapping(self, x, viewNo):
		...
		if viewNo == 2:
			...
			return np.array([elem/norm for elem in x]) # normalization ?
